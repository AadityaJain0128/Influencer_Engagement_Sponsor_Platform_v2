<template>
    <div id="container">
        <h1 align="center">Flag Campaigns</h1>
        <br><br>
            ...
            <div v-if="campaigns.length > 0" class="row">
                <div v-for="c in campaigns" :key="c.id" class="col-md-4 mb-5">
                    <div class="card" style="width: 18rem; align-items: center; text-align: center;">
                        ...
                        <div class="card-body">
                            <button @click="flagCampaign(c.id)" class="btn btn-outline-danger">Flag Campaign</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    export default {
        ...
        methods : {
            async getCampaigns() {
                let { data } = await this.$http.get("/admin/campaigns", {
            ...
            async flagCampaign(id) {
                let { data } = await this.$http.post("/admin/campaigns",
                    { id : id },
                    { headers : { Authorization : `Bearer ${this.$store.state.auth.authToken}` } }
                );
                if (data.status == "success") {
                    this.$store.commit("showAlert", { type : "success", message : "Campaign has been flagged !" });
                    this.getCampaigns();
                }
            },
            clearFilters() {
