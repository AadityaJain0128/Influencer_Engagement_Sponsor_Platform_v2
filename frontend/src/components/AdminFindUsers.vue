            ...
            <div v-if="users.length > 0" class="row">
                <div v-for="user in users" :key="user.username" class="col-md-4 mb-5">
                    <div class="card" style="width: 18rem; align-items: center; text-align: center;">
                        ...
                        <div class="card-body">
                            <button @click="flagUser(user.username)" type="submit" class="btn btn-outline-danger">Flag User</button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    export default {
        ...
        methods : {
            async getUsers() {
                let { data } = await this.$http.get("/admin/users", {
            ...
            async flagUser(username) {
                let { data } = await this.$http.post("/admin/users",
                    { username : username },
                    { headers : { Authorization : `Bearer ${this.$store.state.auth.authToken}` } }
                );
                if (data.status == "success") {
                    this.$store.commit("showAlert", { type : "success", message : "User has been flagged !" });
                    this.getUsers();
                }
            },
            clearFilters() {
